...
| **冷启动到首帧耗时** | **~180 ms - 250 ms** | 1,500 ms - 2,500 ms | 450 ms - 600 ms | 1,800 ms - 3,000 ms | 1,800 ms - 3,500 ms |
| **常驻内存占用 (RSS)** | **~365 MB** (含字体与PDF引擎) | ~627 MB (全进程汇总) | ~266 MB (含WebKit进程) | ~715 MB (全进程汇总) | ~950 MB (全进程汇总) |
| **20KB 技术文档渲染耗时** | **0.88 ms** (0.00088s) | 350 ms - 450 ms | 300 ms - 400 ms | 450 ms - 600 ms | 500 ms - 700 ms |
| **100KB 书籍排版耗时** | **2.81 ms** (30+页纯矢量) | 750 ms - 1,000 ms | 600 ms - 800 ms | 1,000 ms - 1,500 ms | 1,200 ms - 2,000 ms |
...
| :--- | :--- | :---: | :---: | :---: |
| **日常速记 (~1 KB)** | 标题、列表、加粗、代码块 | **0.54 ms** | **0.54 ms** | 13 KB |
| **开源 README (~4 KB)** | 徽标、复杂表格、多级引用 | **0.56 ms** | **0.53 ms** | 13 KB |
| **技术 PRD (~20 KB)** | 复杂数学公式、Mermaid 图表、规范文档 | **0.88 ms** | **1.06 ms** | 34 KB |
| **书籍章节 (~100 KB)** | 20 个完整章节、长文深度排版、30+ 页 | **2.81 ms** | **5.08 ms** | 165 KB |

> **测试结论**：即便面对 100KB、30 页以上的超长技术书籍，SoGoodViewer 从文本输入到输出高精度 PDF 仅耗时 **2.8ms ~ 5ms**。相比人类视觉暂留门槛 (16ms)，用户完全感知不到任何排版等待。
