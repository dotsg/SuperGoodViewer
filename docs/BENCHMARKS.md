...
| 文档                        | 输入    | 输出 PDF |   冷 min |   冷 p50 |   冷 p95 | 编辑 p50 | 热 p50 | A4 分页 冷 p50 | 布局遍数 |
| --------------------------- | ------: | -------: | -------: | -------: | -------: | -------: | -----: | -------------: | -------: |
| `01-note.md` 速记短文           |   137 B |      14 KB |     1.01 | **1.07**|     1.31 |     0.76 |   0.50 |           1.15 |        1 |
| `02-readme.md` 项目 README    | 28.2 KB |    1042 KB |    39.44 | **40.84**|    41.76 |    12.37 |  10.60 |          43.56 |        1 |
| `03-prd.md`（含 6 张 Mermaid）  | 20.1 KB |     211 KB |   178.86 | **182.38**|   189.25 |     9.85 |   4.48 |         185.01 |        1 |
| `04-book.md`（20 章）          | 97.8 KB |     923 KB |   289.11 | **291.93**|   298.53 |    52.40 |  19.56 |         178.94 |        2 |
| `05-real-world.md`          | 21.7 KB |     449 KB |    91.99 | **93.68**|    96.17 |    10.98 |   8.97 |          96.84 |        1 |

### 2.2 阶段拆解（冷口径 p50，单位 ms）

| 文档               | Markdown → Typst 源码 | Typst 布局 + PDF 导出 | 端到端 | 端到端 − 两阶段之和 |
| ------------------ | --------------------: | --------------------: | -----: | ------------------: |
| `01-note.md`       |                  0.01 |                  1.06 |   1.07 |                  ~0 |
| `02-readme.md`     |                  0.13 |                 41.42 |  40.84 |                  ~0 |
| `03-prd.md`        |                138.73 |                 40.04 | 182.38 |                +3.6 |
| `04-book.md`       |                  0.50 |                156.22 | 291.93 |              +135.2 |
| `05-real-world.md` |                 41.50 |                 51.18 |  93.68 |                  ~0 |

两处值得解释的地方：

- **PRD 的 139 ms 全在 Markdown 阶段**，因为该文档有 6 张各不相同的 Mermaid 图，
  冷渲染每张约 23 ms（见 3.2）。Mermaid 冷渲染是首次打开图表类文档的主要成本；
  同一张图第二次出现即命中缓存（约 0.8 µs），所以"编辑"口径骤降到 9.9 ms。
- **书籍章节端到端比两阶段之和多出约 135 ms**，来自 Fluid 切分的第二次布局，详见 2.3。

### 2.3 Fluid 切分为什么要多布局一次

Fluid 模式下，如果整篇文档的自然高度超过 PDF 1.7 的 14,400 pt 上限（实现里留了余量，
按 14,000 pt 判定），`compile_markdown_to_pdf` 会按切分候选重新布局：候选 B 用动态页高
`H / ceil(H / 14000)`，必要时再用固定 14,000 pt 的候选 A 与之对比，取内容完整且画布更小的
那个（`core/src/lib.rs`）。

对 `04-book.md`（自然高度 44,680 pt）逐段计时：

| 阶段                        |    耗时 | 结果                 |
| --------------------------- | ------: | -------------------- |
| 解析 Markdown → Typst 源码  |  1.7 ms | —                    |
| 第 1 遍布局：自然高度       |  143 ms | 1 页 44,680 pt       |
| 候选 A 布局：固定 14,000 pt |  143 ms | 4 页，画布 56,000 pt |
| 候选 B 布局：动态 11,170 pt |  143 ms | 5 页，画布 55,850 pt |
| 溢出扫描（内容完整性检查）  |  ~0 ms  | —                    |
| 导出 PDF                    |   15 ms | 只对胜出者做一次     |

三种页高的布局耗时**完全一样**，所以"巨页更慢"并不成立——切分路径的开销就是实打实多出来
的一整次全文布局，解析与溢出扫描都不值得优化。基准里 `disable_fluid_slice` 的对照证实了
这一点：同一份书稿关掉切分是 157 ms，开着是 292 ms，差 **+135 ms**。

**这次没有优化它，因为便宜的做法都会改变分页结果。** 要算动态页高就得先知道自然高度，
而自然高度只能靠布局得到，所以最少两次布局；唯一能省掉那一次的办法是第一遍直接按
14,000 pt 封顶、再从各页内容底部反推自然高度，那样切片高度变成估算值，长文的分页位置会与
今天不同。"长文一律用固定 14,000 pt"这条捷径同样走不通：测试
`test_fluid_two_pass_auto_height_and_capped_fallback` 里那份 19 KB 文档，动态页高把画布从
28,000 pt 压到 15,674 pt，消掉了 12,326 pt 的空白尾巴。

改为**把成本盯住**：

- `make bench` 总表新增"布局遍数"一列；会切分的文档另外跑一遍 `disable_fluid_slice` 对照，
  在输出里给出"切分开销"，并记入 `benchmark-results.json` 的 `layout_passes` /
  `no_slice_cold`。
- `test_fluid_slicing_layout_pass_counts` 钉死了当前遍数：短文 1 遍、均匀长文 2 遍、
  需要比较两个候选时 3 遍、关掉切分 1 遍。计数由线程局部的
  `compiler::engine::layout_passes()` 提供。

这两条不主张现状最优，只保证以后任何改动都必须明说它把遍数改成了几。

### 2.4 结论

- 短文档（几 KB 以内）冷编译 **约 1 ms**，任何口径下都远低于一帧。
- 20 KB 级真实文档首次打开 **约 94～182 ms**（取决于 Mermaid 图数量），
  再次渲染 **约 4～12 ms**。
- 100 KB 书籍首次打开 **约 0.29 s**，其中 135 ms 是 Fluid 切分的第二次布局（见 2.3）。
...
| --------------------------------------------------------------------- | -------: | -------: | -------: | ---------------: |
| `E = mc^2`                                                            |     2.30 | **4.00** |     6.10 |          250,000 |
| `\int_{-\infty}^{\infty} e^{-x^2} dx = \sqrt{\pi}`                    |     5.40 | **7.10** |     9.50 |          141,000 |
| `\nabla \times \mathbf{B} - \frac{1}{c}\frac{\partial \mathbf{E}}{\partial t} = \frac{4\pi}{c}\mathbf{J}` | 6.10 | **7.20** | 8.70 |  139,000 |
| 3×3 `pmatrix`                                                         |     3.40 | **3.80** |     4.10 |          263,000 |

即 **13.9 万 ~ 25 万 式/秒**（旧文档写 15 万 ~ 32 万，取的是均值/最优值）。
...
| 口径                     |      min |      p50 |      p95 | 说明                                     |
| ------------------------ | -------: | -------: | -------: | ---------------------------------------- |
| 冷渲染（每次图内容不同） | 22.14 ms | **22.87 ms** | 23.90 ms | 真正的解析 + 布局 + SVG 生成         |
| 缓存命中（同一张图）     |   708 ns | **792 ns**   |  959 ns | SHA-256 求哈希 + 哈希表查表          |

旧文档把 829 ns 写成"增量渲染"，容易被读成"再渲染一次只要 0.8 µs"。
准确表述是：**命中缓存时不再渲染**，0.8 µs 是查表成本。冷渲染 22.9 ms
是图表类文档首屏耗时的主要来源。
...
通用版（arm64 + x86_64）同样重新打包实测，重复的引擎在通用版里是双份，所以省得更多：

| 口径                       | 瘦身前 (v1.0.8) |       现在 |     降幅 |
| -------------------------- | --------------: | ---------: | -------: |
| 通用版 app bundle          |          248 MB | **154 MB** |    −38 % |
| 通用版 DMG（压缩后下载体积）|        121.6 MB | **76.1 MB** |    −37 % |

> 瘦身前的两个数分别是 `/Applications` 里已安装的 v1.0.8（`du -sm`）与 GitHub Releases
> 上 v1.0.8 的 `SuperGoodViewer-v1.0.8-macos.dmg`（127,490,872 B）；现在的两个数来自本分支
> `make dmg` 的产物，主执行文件、dylib 与 CLI 均已 `lipo -info` 确认为 x86_64 + arm64 双架构。

> **另一处更正**：本文一度写 "DMG 50 MB"，那是仓库根目录一个 2026-09-14 的旧文件
> （52,783,827 B，对应 v1.0.5 时期），不是 v1.0.8。
>
> 旧文档的 94 MB 同样不成立，即使去掉重复的 CLI，arm64 版也还有 107 MB。
>
> Windows 与 Linux 的发行包里同样打包了 `sgv-cli`，这次瘦身对它们一样生效，
> 但本轮没有在对应平台上打包复测。
...
| Obsidian           | 482 MB |
| MarkText           | 368 MB |
| SuperGoodViewer    | 107 MB（arm64）/ 154 MB（通用版）|
...
| 进程数          | **1**                                         | 4                  | 2                | 5                  | 8+                |
| 安装体积        | **107 MB (arm64) / 154 MB (通用)**            | 482 MB             | 46 MB            | 368 MB             | 933 MB            |
...
| 同文档重新渲染  | **9.0 ms**                                    | —                  | —                | —                  | —                 |
| 100 KB 书籍首次排版 | **292 ms**                                | 750 ~ 1,000 ms     | 600 ~ 800 ms     | 1,000 ~ 1,500 ms   | 1,200 ~ 2,000 ms  |
