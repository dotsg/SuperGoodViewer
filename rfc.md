...
### 3.1 双排版模型（Dual-Layout）支持
阅读器不应强制拘泥于固定的 A4 纸张，需在 UI 层面提供一键切换：
1. **连续流式视窗视图（Fluid Screen Mode - 默认）**：
   页面宽度自适应窗口逻辑宽度，高度 `auto`，消除分页缝隙，呈现如同原生应用般的无缝长卷轴滚动体验。
2. **出版打印视图（Print / Paged Mode）**：
   标准 A4/Letter 纸张，带页眉页脚、动态页码与孤行控制，提供 100% 打印即所见效果。
