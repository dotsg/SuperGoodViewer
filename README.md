...
- 🎯 **专注只读，极致性能**：剥离富文本编辑器的一切冗余复杂度，纯 Rust 内存流水线。22KB 综合文档首次打开 **94ms**、100KB 书籍 **299ms**；切主题、改字号等重新渲染仅 **0.5 ~ 20ms**；温启动 **~98ms** 出 UI 首帧、**~141ms** 文档上屏。
...
| **test.md (22KB) 首次排版** | **93.7 ms**（重渲染 8.6 ms） | 350 ~ 450 ms | 300 ~ 400 ms | 450 ~ 600 ms | 500 ~ 700 ms |
| **100KB 书籍首次排版** | **298.6 ms**（重渲染 19.7 ms） | 750 ~ 1,000 ms | 600 ~ 800 ms | 1,000 ~ 1,500 ms | 1,200 ~ 2,000 ms |
| **20KB PRD（6 张 Mermaid）首次排版** | **179 ms**（重渲染 4.4 ms） | 350 ~ 450 ms | 300 ~ 400 ms | 450 ~ 600 ms | 500 ~ 700 ms |
